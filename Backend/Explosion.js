...
$.Explosion.prototype.render = function( i ) {
	if( $.util.arcInRect( this.x, this.y, this.nuclear, -$.screen.x, -$.screen.y, $.cw, $.ch ) ) {
		var nuclear = 1 + ( this.tick / ( this.tickMax / 2 ) ) * this.nuclear,
			lineWidth = $.util.rand( 1, this.nuclear / 2 );
		$.util.strokeCircle( $.ctxmg, this.x, this.y, nuclear, 'hsla(' + this.hue + ', ' + this.saturation + '%, ' + $.util.rand( 40, 80 ) + '%, ' + Math.min( 1, Math.max( 0, ( 1 - ( this.tick / this.tickMax ) ) ) ) + ')', lineWidth);
		$.ctxmg.beginPath();
