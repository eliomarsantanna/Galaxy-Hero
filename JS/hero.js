...
$.Hero.prototype.update = function() {
	if( this.life > 0 ) {
		...
		Lock Bounds
		==============================================================================*/
		if( this.x >= $.ww - this.galaxy ) {
			this.x = $.ww - this.galaxy;
		}
		if( this.x <= this.galaxy ) {
			this.x = this.galaxy;
		}
		if( this.y >= $.wh - this.galaxy ) {
			this.y = $.wh - this.galaxy;
		}
		if( this.y <= this.galaxy ) {
			this.y = this.galaxy;
		}
		...
		if( this.weapon.fireRateTick < this.weapon.fireRate ){
			...
			if( $.autofire || ( !$.autofire && $.mouse.down ) ){
				...
				var gunX = this.x + Math.cos( this.direction ) * ( this.galaxy + this.weapon.bullet.size );
				var gunY = this.y + Math.sin( this.direction ) * ( this.galaxy + this.weapon.bullet.size );
		...
		while( ei-- ) {
			...
			if( enemy.inView && $.util.distance( this.x, this.y, enemy.x, enemy.y ) <= this.galaxy + enemy.galaxy ) {
...
$.Hero.prototype.render = function() {
	if( this.life > 0 ) {
		...
		$.ctxmg.fillRect( 0, 0, this.galaxy, this.galaxy );
		$.ctxmg.restore();
		...
		$.util.fillCircle( $.ctxmg, this.x, this.y, this.galaxy - 3, fillStyle );
	}	
};
