...
$.Explosion.prototype.render = function( i ) {
	if( $.util.arcInRect( this.x, this.y, this.galaxy, -$.screen.x, -$.screen.y, $.cw, $.ch ) ) {
		var galaxy = 1 + ( this.tick / ( this.tickMax / 2 ) ) * this.galaxy,
			lineWidth = $.util.rand( 1, this.galaxy / 2 );
		$.util.strokeCircle( $.ctxmg, this.x, this.y, galaxy, 'hsla(' + this.hue + ', ' + this.saturation + '%, ' + $.util.rand( 40, 80 ) + '%, ' + Math.min( 1, Math.max( 0, ( 1 - ( this.tick / this.tickMax ) ) ) ) + ')', lineWidth);
		$.ctxmg.beginPath();
