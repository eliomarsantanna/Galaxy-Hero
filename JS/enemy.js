...
$.Enemy.prototype.update = function( i ) {
	...
	Lock Bounds
	==============================================================================*/
	if( this.lockBounds && !$.util.arcInRect( this.x, this.y, this.galaxy + 10, 0, 0, $.ww, $.wh ) ) {
		$.enemies.splice( i, 1 );
	}
	...
	Update View
	==============================================================================*/
	if( $.util.arcInRect( this.x, this.y, this.galaxy, -$.screen.x, -$.screen.y, $.cw, $.ch ) ) {
		this.inView = 1;
	} else {
		this.inView = 0;
	}
...
$.Enemy.prototype.renderHealth = function( i ) {
	if( this.inView && this.life > 0 && this.life < this.lifeMax ) {
		...
		$.ctxmg.fillRect( this.x - this.galaxy, this.y - this.galaxy - 6, this.galaxy * 2, 3 );
		$.ctxmg.fillStyle = 'hsla(' + ( this.life / this.lifeMax ) * 120 + ', 100%, 50%, 0.75)';	
		$.ctxmg.fillRect( this.x - this.galaxy, this.y - this.galaxy - 6, ( this.galaxy * 2 ) * ( this.life / this.lifeMax ), 3 );
	}
};
...
$.Enemy.prototype.render = function( i ) {
	if( this.inView ) {
		...
		$.util.fillCircle( $.ctxmg, this.x, this.y, this.galaxy, this.fillStyle );
		$.util.strokeCircle( $.ctxmg, this.x, this.y, this.galaxy / 4 + Math.cos( mod ) * this.galaxy / 4, this.strokeStyle, 1.5 );
		$.util.strokeCircle( $.ctxmg, this.x, this.y, this.galaxy - 0.5, this.strokeStyle, 1 );
		
		$.ctxmg.strokeStyle = this.strokeStyle;
		$.ctxmg.lineWidth = 4;
		$.ctxmg.beginPath();
		$.ctxmg.arc( this.x, this.y, this.galaxy - 0.5, mod + $.pi, mod + $.pi + $.pi / 2 );		
		$.ctxmg.stroke();
		$.ctxmg.beginPath();
		$.ctxmg.arc( this.x, this.y, this.galaxy - 0.5, mod, mod + $.pi / 2 );		
		$.ctxmg.stroke();

		if( $.slow) {
			$.util.fillCircle( $.ctxmg, this.x, this.y, this.galaxy, 'hsla(' + $.util.rand( 160, 220 ) + ', 100%, 50%, 0.25)' );
		} 
		if( this.hitFlag > 0 ) {
			this.hitFlag -= $.dt;
			$.util.fillCircle( $.ctxmg, this.x, this.y, this.galaxy, 'hsla(' + this.hue + ', ' + this.saturation + '%, 75%, ' + this.hitFlag / 10 + ')' );
			$.util.strokeCircle( $.ctxmg, this.x, this.y, this.galaxy, 'hsla(' + this.hue + ', ' + this.saturation + '%, ' + $.util.rand( 60, 90) + '%, ' + this.hitFlag / 10 + ')', $.util.rand( 1, 10) );	
		}
		this.renderHealth();
	}
};
